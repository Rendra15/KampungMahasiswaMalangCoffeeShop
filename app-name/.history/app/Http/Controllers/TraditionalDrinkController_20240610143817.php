...
class TraditionalDrinkController extends Controller
{
    ...
    public function getRating(Request $request, $TraditionalDrinkId)
    {
        ...
        $TraditionalDrink = Snack::find($TraditionalDrinkId);
        if ($Snack) {
            if ($Snack->$columnName !== null) {
                ...
                Log::info('No rating found for Snack', ['TraditionalDrinkId' => $TraditionalDrinkId, 'column' => $columnName]);
                return response()->json(['error' => 'No rating found for FoSnackod'], 404);
            }
        } else {
            Log::info('Snack not found', ['TraditionalDrinkId' => $TraditionalDrinkId]);
