...
class CoffeeController extends Controller
{
    ...
    {
        // Lakukan logika untuk mendapatkan rating berdasarkan coffeeId
        $coffee = Coffee::find($coffeeId);
        if ($coffee) {
            // Pastikan bahwa nama kolom dinamis adalah valid
            $columnName = $request->input('columnName');

            // Periksa apakah nama kolom dinamis ada di model Coffee
            if ($coffee->$columnName !== null) {
                // Ambil nilai rating dari kolom dinamis yang sesuai
                $rating = $coffee->$columnName;

                // Logika untuk menampilkan informasi Coffee (sesuaikan sesuai kebutuhan Anda)
                // $coffee->show();

                // Log info bahwa rating ditemukan
                Log::info('Rating found', ['coffee_id' => $coffeeId, 'rating' => $rating]);

                // Kembalikan respons JSON dengan rating yang ditemukan
                return response()->json(['rating' => $rating]);
            } else {
                // Jika nama kolom dinamis tidak valid atau nilai rating null, log pesan kesalahan
                Log::error('Invalid column name or null rating', ['column_name' => $columnName]);

                // Kembalikan respons JSON dengan pesan kesalahan
                return response()->json(['error' => 'Invalid column name or null rating'], 400);
            }
        } else {
            // Jika objek Coffee tidak ditemukan, log pesan kesalahan
