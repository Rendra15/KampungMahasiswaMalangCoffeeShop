    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data && data.success) {
                setCookie('rating_' + columnName, rating, 7);
...
function loadRatingFromCookie() {
    // Loop melalui setiap produk
    for (let i of products.datakopi) {
        let productName = i.productName.toLowerCase();
        let validId = productName.replace(/\s+/g, '_');

        // Cek apakah ada nilai rating yang disimpan dalam cookie
        let storedRating = getCookie('rating_' + validId);
        if (storedRating !== null) {
            // Jika ada, perbarui nilai bintang sesuai dengan nilai yang disimpan
            ratings[validId] = parseInt(storedRating);
            let ratingDiv = document.getElementById(validId).nextElementSibling;
            if (ratingDiv) {
...
// Panggil fungsi loadRatingFromCookie saat halaman dimuat kembali
document.addEventListener('DOMContentLoaded', loadRatingFromCookie);
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.success) {
                ...
                let storedEmail = getCookie('user_email');
                if (storedEmail && storedEmail !== email) {
                    // Email has changed, erase rating cookies
                    eraseCookie('user_email');
                    document.cookie.split(';').forEach(function(c) {
                        if (c.trim().startsWith('rating_')) {
                            eraseCookie(c.trim().split('=')[0]);
                        }
                    });
                }

                // Set the new email in the cookie
                setCookie('user_email', email, 7); // Set cookie with new email
