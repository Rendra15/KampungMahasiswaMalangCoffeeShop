            ...
            if (cardProductName === productName) {
                ...
                for (let j = 1; j <= 5; j++) {
                    ...
                    star.addEventListener('click', function() {
                        ...
                        ratings[validId] = rating; // Save the rating
                        console.log(rating)
                        updateStars(rating);
                        sendRatingToServer(validId, rating);
                    });
                    ratingDiv.appendChild(star);
                    ratingDiv.style.fontStyle="normal"

                }

                // Load the stored rating for the product if it exists
                if (ratings[validId]) {
                    updateStars(ratings[validId]);
                }
...
function sendRatingToServer(productId, columnName, rating) {
    debugger
    console.log((productId, columnName, rating)
    fetch(`/coffee/${productId}/rating`, {
        ...
        body: JSON.stringify({ productId: columnName, rating: rating })
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        // Assuming the server responds with { success: true } upon successful rating submission
        if (data.success) {
