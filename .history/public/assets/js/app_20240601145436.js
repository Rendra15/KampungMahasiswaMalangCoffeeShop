...
function sendRatingToServer(columnName, rating) {
    ...
    $.ajax({
        url:`/Menu-Coffee/${columnName}/rating`,  // Replace '1' with the actual coffee ID
        type: 'POST',
...
for (let i of products.datakopi) {
    ...
    let validId = words.map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join('_');
    name.id = validId;
